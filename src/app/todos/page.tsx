...
export default function TodosPage() {
  ...
  const stats = {
    ...
    overdue: todos.filter((t: Todo) => !t.completed && isOverdue(t.dueDate)).length,
    dueToday: todos.filter((t: Todo) => !t.completed && isDueToday(t.dueDate)).length
  }
  ...
  return (
    <div className="space-y-6">
            ...
            {filteredTodos.map((todo: Todo) => (
              ...
              >
                <div className="flex items-center gap-4">
                    ...
                    {editingId === todo.id ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={saveEdit} className="px-3 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                          <Save size={16} />
                        </button>
                        <button onClick={cancelEdit} className="px-3 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
                          <X size={16} />
                        </button>
                      </div>
                    ) : (
                  ...
                  {editingId !== todo.id && (
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(todo)} className="text-gray-400 hover:text-blue-600 transition-colors">
                        <Edit3 size={18} />
                      </button>
                      <button onClick={() => handleDeleteTodo(todo.id)} className="text-gray-400 hover:text-red-600 transition-colors">
                        <X size={18} />
                      </button>
                    </div>
                  )}
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-2">
              <button onClick={() => navigateMonth('prev')} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
                <ChevronLeft size={20} />
              </button>
              <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1 text-sm text-blue-600 hover:bg-blue-50 rounded-lg">
                Today
              </button>
              <button onClick={() => navigateMonth('next')} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
          ...
          <div className="grid grid-cols-7 gap-1">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} className="p-2 text-center text-sm font-medium text-gray-500 border-b">{day}</div>
            ))}
            ...
            {(() => {
              ...
              for (let day = 1; day <= daysInMonth; day++) {
                ...
                days.push(
                  <div key={day} className={`h-24 p-1 border border-gray-100 ${isToday ? 'bg-blue-50 border-blue-200' : ''}`}>
                    ...
                    <div className="space-y-1 max-h-16 overflow-y-auto">
                      {dayTodos.slice(0, 3).map((todo: Todo) => (
                        <div
                          ...
                          className={`text-xs p-1 rounded truncate cursor-pointer ${
                            todo.completed ? 'bg-green-100 text-green-700 line-through' : priorityColors[todo.priority]
                          }`}
                          onClick={() => handleToggleComplete(todo.id)}
                          title={todo.text}
                        >
                      ...
                      {dayTodos.length > 3 && (
                        <div className="text-xs text-gray-500 font-medium">+{dayTodos.length - 3} more</div>
                      )}
                    </div>
                  </div>
                )
      ...
      {/* Bulk Import Modal */}
      {showBulkImport && (
        <BulkImport feature="todos" onImport={handleBulkImport} onClose={() => setShowBulkImport(false)} />
      )}
    </div>
  )
}
